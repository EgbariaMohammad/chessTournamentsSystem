#ifndef _CHESS_SYSTEM_H
#define _CHESS_SYSTEM_H
 ...
 * @param tournamentID - new tournament id. Must be positive, and unique.
 * @param maxGamesPerPlayer - maximum number of games a player is allow to play in the specified tournament.
 *                               Must be positive/
 * @param tournamentLocation - location in which the tournament take place. Must be non-empty.
 *
 * @return
 *     CHESS_NULL_ARGUMENT - if chess/tournamentLocation are NULL.
 ...
 */
ChessResult chessAddTournament(ChessSystem chess, int tournamentID,
                                int maxGamesPerPlayer, const char* tournamentLocation);

/**
 * chessAddGame: add a new match to a chess tournament.
 *
 * @param chess - chess system that contains the tournament. Must be non-NULL.
 * @param tournamentID - the tournament id. Must be positive, and unique.
 * @param firstPlayer - first player id. Must be positive.
 * @param secondPlayer - second player id. Must be positive.
 ...
 * @param playTime - duration of the match in seconds. Must be non-negative.
 *
 * @return
 ...
 */
ChessResult chessAddGame(ChessSystem chess, int tournamentID, int firstPlayer,
                         int secondPlayer, Winner winner, int playTime);
 ...
 * @param playerID - the player id. Must be positive.
 *
 * @return
 *     CHESS_NULL_ARGUMENT - if chess is NULL.
 ...
 */
ChessResult chessRemovePlayer(ChessSystem chess, int playerID);
 ...
 */
ChessResult chessEndTournament(ChessSystem chess, int tournamentID);
 ...
 * @param playerID - player ID. Must be positive.
 * @param chessResult - this variable will contain the returned error code.
 * @return
 *     CHESS_NULL_ARGUMENT - if chess is NULL.
 *     CHESS_INVALID_ID - if the player ID number is invalid.
 *     CHESS_PLAYER_NOT_EXIST - if the player does not exist in the system.
 ...
 *
 * @param chess - a chess system. Must be non-NULL.
 * @param pathFile - the file path which within it the tournament statistics will be saved.
 * @return
 *     CHESS_NULL_ARGUMENT - if chess is NULL.
 ...
 *     CHESS_SUCCESS - if the ratings was printed successfully.
 */
ChessResult chessSaveTournamentStatistics(ChessSystem chess, char* pathFile);

#endif // CHESS_SYSTEM_H
